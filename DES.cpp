...
using namespace std;
 ...
 string DES::encode(string str, string key) {
    ...
    while (count * 8 < s) {
        ...
        fill(sub);
        if (DEBUG) {
            cout << "code length: " << tempCode_1.size() << "\n";
        }
        if (DEBUG) {
            cout << "IP0" << "\n";
        }
...
string DES::Feistel(string R, string K) {
    ...
    if (DEBUG) {
        cout << "substring length: " << rec.size() << "\n";
    }
...
bool DES::deleteFile(string filePath)
{
...
int DES::chekkey(string key) {
    ...
    {
        cerr << "The key is too short!\n";
        return 1;
    }
   
}
int DES::chekstring(string str) {
    ...
    if (longs == 64) {
        ...
        cerr << "The chifrtext is too short!\n";
        return 1;
    }
    else if (longs > 64) {
        cerr << "The chifrtext is too long!\n";
        return -1;
    }
...
DES::DES(string inputfile, string outputfile)
{
    int mode = 2;
    this->mode = mode;
...
DES::DES(string inputfile, string outputfile, string keyfile)
{
    int mode = 3;
    this->mode = mode;
...
int DES::read_Datafile(int cryptomode, string key)
{
    switch (mode)
    {
    case 1:
...
int DES::read_Datafile(int cryptomode)
{
    if (mode == 3) {
        if (cryptomode == 0) {
